...
def word_count(input_path, output_path):
    ...
    if output_path is not None:
        t_env.create_temporary_table(
            "sink",
            TableDescriptor.for_connector("filesystem")
            .schema(Schema.new_builder().column("word", DataTypes.STRING()).column("count", DataTypes.BIGINT()).build())
    ...
    # compute word count
    tab.flat_map(split).alias("word").group_by(col("word")).select(col("word"), lit(1).count).execute_insert(
        "sink"
    ).wait()
